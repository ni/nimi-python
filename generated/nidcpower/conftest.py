"""Diagnostic conftest.py — generated for nidcpower system tests.
...
def _log_proc_status_threads():
    """Log kernel-level thread count from /proc/self/status."""
    try:
...
def _atexit_callback():
    """Log thread state at the start of the Python atexit phase."""
...
def _diagnostic_logging():
    """Diagnostic fixture active for all nidcpower system tests.
    ...
    #    - stdout is used because pytest -s passes it directly to the CI
    #      build log, whereas stderr routing varies by runner.
    #    - faulthandler uses raw write() syscalls — safe from signal handlers.
    #    - enable() covers SIGFPE, SIGABRT, SIGSEGV, SIGBUS, SIGILL; do NOT
    #      call faulthandler.register() for those — it raises RuntimeError.
    # ------------------------------------------------------------------
