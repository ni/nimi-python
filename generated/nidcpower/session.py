...
class Session(_SessionBase):
    ...
    def export_signal(self, signal, output_terminal, signal_identifier=''):
        ...
        **Related Topics:**
        ...
        Args:
            signal (enums.Signals): Specifies which trigger or event to export.
                **Defined Values:**
            ...
            output_terminal (string): Specifies where to export the selected signal.
                **Relative Terminals**:
                ...
                +-------------+----------------------+
        '''
        if type(signal) is not enums.Signals:
            raise TypeError('Parameter mode must be of type ' + str(enums.Signals))
        error_code = self._library.niDCPower_ExportSignal(self._vi, signal.value, signal_identifier.encode(self._encoding), output_terminal.encode(self._encoding))
        errors.handle_error(self, error_code, ignore_warnings=False, is_error_handling=False)
        return
    ...
    def wait_for_event(self, event_id, timeout=10.0):
        ...
        error_code = self._library.niDCPower_WaitForEvent(self._vi, event_id.value, timeout)
        errors.handle_error(self, error_code, ignore_warnings=False, is_error_handling=False)
        return
