...
class struct_niScope_wfmInfo(ctypes.Structure):  # noqa N801
    ...
    def __init__(
        self,
        data=None,
        absolute_initial_x=0.0,
        relative_initial_x=0.0,
        x_increment=0.0,
        actual_samples=0,
        offset=0.0,
        gain=0.0,
...
class WaveformInfo(object):
    ...
    def __str__(self):
        ...
        ):  # We explicitly look for not None to differentiate from empty string
            string_representation += row_format_s.format("Channel", self.channel)
        if (
            self.record is not None
        ):  # We explicitly look for not None to differentiate from 0
            string_representation += row_format_d.format("Record", self.record)

        string_representation += row_format_g.format(
            "Absolute X0", self.absolute_initial_x
        )
        string_representation += row_format_g.format(
            "Relative X0", self.relative_initial_x
        )
        ...
        ):  # We explicitly look for not None to differentiate from empty array
            string_representation += row_format_g.format(
                "Waveform Length", len(self.samples)
            )

        # Put possible private variable last
        if (
            self._actual_samples is not None
        ):  # We explicitly look for not None to differentiate from 0
            string_representation += row_format_d.format(
                "_actual samples", self._actual_samples
            )
