...
class SideEffectsHelper(object):
    ...
    def niSE_ExpandRouteSpec(
        ...
        if self._defaults["ExpandRouteSpec"]["expandedRouteSpec"] is None:
            raise MockFunctionCallError(
                "niSE_ExpandRouteSpec", param="expandedRouteSpec"
            )
    ...
    def niSE_FindRoute(
        ...
        if path_capability is not None:
            path_capability.contents.value = self._defaults["FindRoute"][
                "pathCapability"
            ]
        return self._defaults["FindRoute"]["return"]
    ...
    def set_side_effects_and_return_values(self, mock_library):
        mock_library.niSE_CloseSession.side_effect = MockFunctionCallError(
            "niSE_CloseSession"
        )
        ...
        mock_library.niSE_ConnectAndDisconnect.side_effect = MockFunctionCallError(
            "niSE_ConnectAndDisconnect"
        )
        mock_library.niSE_ConnectAndDisconnect.return_value = 0
        mock_library.niSE_Disconnect.side_effect = MockFunctionCallError(
            "niSE_Disconnect"
        )
        mock_library.niSE_Disconnect.return_value = 0
        mock_library.niSE_DisconnectAll.side_effect = MockFunctionCallError(
            "niSE_DisconnectAll"
        )
        mock_library.niSE_DisconnectAll.return_value = 0
        mock_library.niSE_ExpandRouteSpec.side_effect = MockFunctionCallError(
            "niSE_ExpandRouteSpec"
        )
        mock_library.niSE_ExpandRouteSpec.return_value = 0
        mock_library.niSE_FindRoute.side_effect = MockFunctionCallError(
            "niSE_FindRoute"
        )
        mock_library.niSE_FindRoute.return_value = 0
        mock_library.niSE_GetAllConnections.side_effect = MockFunctionCallError(
        ...
        mock_library.niSE_IsConnected.side_effect = MockFunctionCallError(
            "niSE_IsConnected"
        )
        mock_library.niSE_IsConnected.return_value = 0
        mock_library.niSE_IsDebounced.side_effect = MockFunctionCallError(
            "niSE_IsDebounced"
        )
        mock_library.niSE_IsDebounced.return_value = 0
        mock_library.niSE_OpenSession.side_effect = MockFunctionCallError(
            "niSE_OpenSession"
        )
        mock_library.niSE_OpenSession.return_value = 0
        mock_library.niSE_WaitForDebounce.side_effect = MockFunctionCallError(
