...
class Library(object):
    ...
    def niSE_CloseSession(self, vi):  # noqa: N802
        with self._func_lock:
            if self.niSE_CloseSession_cfunc is None:
                self.niSE_CloseSession_cfunc = self._get_library_function(
                    "niSE_CloseSession"
                )
    ...
    def niSE_Connect(
        ...
        return self.niSE_Connect_cfunc(
            vi, connect_spec, multiconnect_mode, wait_for_debounce
        )

    def niSE_ConnectAndDisconnect(
        self,
        vi,
        connect_spec,
        disconnect_spec,
        ...
        with self._func_lock:
            if self.niSE_ConnectAndDisconnect_cfunc is None:
                self.niSE_ConnectAndDisconnect_cfunc = self._get_library_function(
                    "niSE_ConnectAndDisconnect"
                )
                self.niSE_ConnectAndDisconnect_cfunc.argtypes = [
                    ViSession,
                    ctypes.POINTER(ViChar),
                    ctypes.POINTER(ViChar),
                    ViInt32,
        ...
        return self.niSE_ConnectAndDisconnect_cfunc(
            vi,
            connect_spec,
            disconnect_spec,
            multiconnect_mode,
    ...
    def niSE_Disconnect(self, vi, disconnect_spec):  # noqa: N802
        with self._func_lock:
            if self.niSE_Disconnect_cfunc is None:
                self.niSE_Disconnect_cfunc = self._get_library_function(
                    "niSE_Disconnect"
                )
                self.niSE_Disconnect_cfunc.argtypes = [
                    ViSession,
                    ctypes.POINTER(ViChar),
                ]  # noqa: F405
    ...
    def niSE_DisconnectAll(self, vi):  # noqa: N802
        with self._func_lock:
            if self.niSE_DisconnectAll_cfunc is None:
                self.niSE_DisconnectAll_cfunc = self._get_library_function(
                    "niSE_DisconnectAll"
                )
    ...
    def niSE_ExpandRouteSpec(
        ...
        with self._func_lock:
            if self.niSE_ExpandRouteSpec_cfunc is None:
                self.niSE_ExpandRouteSpec_cfunc = self._get_library_function(
                    "niSE_ExpandRouteSpec"
                )
                self.niSE_ExpandRouteSpec_cfunc.argtypes = [
                    ViSession,
                    ctypes.POINTER(ViChar),
                    ViInt32,
                    ctypes.POINTER(ViChar),
        ...
        return self.niSE_ExpandRouteSpec_cfunc(
            vi, route_spec, expand_action, expanded_route_spec, expanded_route_spec_size
        )

    def niSE_FindRoute(
        self, vi, channel1, channel2, route_spec, route_spec_size, path_capability
    ):  # noqa: N802
        with self._func_lock:
            if self.niSE_FindRoute_cfunc is None:
                self.niSE_FindRoute_cfunc = self._get_library_function("niSE_FindRoute")
                self.niSE_FindRoute_cfunc.argtypes = [
                    ViSession,
                    ctypes.POINTER(ViChar),
                    ctypes.POINTER(ViChar),
                    ctypes.POINTER(ViChar),
        ...
        return self.niSE_FindRoute_cfunc(
            vi, channel1, channel2, route_spec, route_spec_size, path_capability
        )

    def niSE_GetAllConnections(self, vi, route_spec, route_spec_size):  # noqa: N802
        with self._func_lock:
            if self.niSE_GetAllConnections_cfunc is None:
                self.niSE_GetAllConnections_cfunc = self._get_library_function(
                    "niSE_GetAllConnections"
                )
                self.niSE_GetAllConnections_cfunc.argtypes = [
                    ViSession,
                    ctypes.POINTER(ViChar),
                    ctypes.POINTER(ViInt32),
                ]  # noqa: F405
    ...
    def niSE_GetError(
        ...
        return self.niSE_GetError_cfunc(
            vi, error_number, error_description, error_description_size
        )

    def niSE_IsConnected(self, vi, route_spec, is_connected):  # noqa: N802
        with self._func_lock:
            if self.niSE_IsConnected_cfunc is None:
                self.niSE_IsConnected_cfunc = self._get_library_function(
                    "niSE_IsConnected"
                )
                self.niSE_IsConnected_cfunc.argtypes = [
                    ViSession,
                    ctypes.POINTER(ViChar),
    ...
    def niSE_IsDebounced(self, vi, is_debounced):  # noqa: N802
        with self._func_lock:
            if self.niSE_IsDebounced_cfunc is None:
                self.niSE_IsDebounced_cfunc = self._get_library_function(
                    "niSE_IsDebounced"
                )
                self.niSE_IsDebounced_cfunc.argtypes = [
                    ViSession,
                    ctypes.POINTER(ViBoolean),
                ]  # noqa: F405
    ...
    def niSE_OpenSession(self, virtual_device_name, option_string, vi):  # noqa: N802
        with self._func_lock:
            if self.niSE_OpenSession_cfunc is None:
                self.niSE_OpenSession_cfunc = self._get_library_function(
                    "niSE_OpenSession"
                )
                self.niSE_OpenSession_cfunc.argtypes = [
                    ctypes.POINTER(ViChar),
                    ctypes.POINTER(ViChar),
    ...
    def niSE_WaitForDebounce(self, vi, maximum_time_ms):  # noqa: N802
        with self._func_lock:
            if self.niSE_WaitForDebounce_cfunc is None:
                self.niSE_WaitForDebounce_cfunc = self._get_library_function(
                    "niSE_WaitForDebounce"
                )
                self.niSE_WaitForDebounce_cfunc.argtypes = [
                    ViSession,
                    ViInt32,
                ]  # noqa: F405
