...
class SideEffectsHelper(object):
    ...
    def niDMM_FetchMultiPoint(self, vi, maximum_time, array_size, reading_array, actual_number_of_points):  # noqa: N802
        ...
        for i in range(min(len(reading_array), len(self._defaults['FetchMultiPoint']['readingArray']))):
            reading_array[i] = self._defaults['FetchMultiPoint']['readingArray'][i].encode('ascii')
    ...
    def niDMM_FetchWaveform(self, vi, maximum_time, array_size, waveform_array, actual_number_of_points):  # noqa: N802
        ...
        for i in range(min(len(waveform_array), len(self._defaults['FetchWaveform']['waveformArray']))):
            waveform_array[i] = self._defaults['FetchWaveform']['waveformArray'][i].encode('ascii')
    ...
    def niDMM_ReadWaveform(self, vi, maximum_time, array_size, waveform_array, actual_number_of_points):  # noqa: N802
        ...
        for i in range(min(len(waveform_array), len(self._defaults['ReadWaveform']['waveformArray']))):
            waveform_array[i] = self._defaults['ReadWaveform']['waveformArray'][i].encode('ascii')
