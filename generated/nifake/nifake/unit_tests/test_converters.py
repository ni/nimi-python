...
def test_convert_init_with_options_dictionary():
    ...
    assert (
        _converters.convert_init_with_options_dictionary("Simulate=1") == "Simulate=1"
    )
    assert (
        _converters.convert_init_with_options_dictionary(
            {
                "Simulate": True,
            }
        )
    ...
    assert (
        _converters.convert_init_with_options_dictionary(
            {"Simulate": True, "Cache": False}
        )
        == "Cache=0,Simulate=1"
    )
    assert (
        _converters.convert_init_with_options_dictionary(
            {"DriverSetup": {"Model": "5162 (4CH)", "Bitfile": "CustomProcessing"}}
        )
        == "DriverSetup=Bitfile:CustomProcessing;Model:5162 (4CH)"
    )
    assert (
        _converters.convert_init_with_options_dictionary(
            {
        ...
        == "DriverSetup=Bitfile:CustomProcessing;Model:5162 (4CH),Simulate=1"
    )
    assert (
        _converters.convert_init_with_options_dictionary(
            {"simulate": True, "cache": False}
    ...
    assert (
        _converters.convert_init_with_options_dictionary(
            {"driver_setup": {"Model": "5162 (4CH)", "Bitfile": "CustomProcessing"}}
        )
        == "DriverSetup=Bitfile:CustomProcessing;Model:5162 (4CH)"
    )
    assert (
...
def test_convert_timedelta_to_seconds_double():
    test_result = _converters.convert_timedelta_to_seconds_real64(
        hightime.timedelta(seconds=10)
    )
    assert test_result.value == 10.0
    assert isinstance(test_result, _visatype.ViReal64)
    test_result = _converters.convert_timedelta_to_seconds_real64(
        hightime.timedelta(nanoseconds=-0.5)
    )
...
def test_convert_timedelta_to_milliseconds_int32():
    test_result = _converters.convert_timedelta_to_milliseconds_int32(
        hightime.timedelta(seconds=10)
    )
    assert test_result.value == 10000
    assert isinstance(test_result, _visatype.ViInt32)
    test_result = _converters.convert_timedelta_to_milliseconds_int32(
        hightime.timedelta(seconds=-5)
    )
...
def test_convert_timedeltas_to_seconds_real64():
    ...
    test_input = [
        hightime.timedelta(seconds=10.5),
        hightime.timedelta(nanoseconds=-0.5),
    ]
...
def test_repeated_capabilities_list_prefix():
    test_result_list = _converters.convert_repeated_capabilities(
        ["ScriptTrigger0", "ScriptTrigger1"], prefix="ScriptTrigger"
    )
...
def test_repeated_capabilities_raw():
    test_result_list = _converters.convert_repeated_capabilities(
        r"ScriptTrigger0,ScriptTrigger1", prefix="ScriptTrigger"
    )
    assert test_result_list == ["ScriptTrigger0", "ScriptTrigger1"]
    test_result_list = _converters.convert_repeated_capabilities(
        r"ScriptTrigger0,ScriptTrigger1", prefix=r"ScriptTrigger"
    )
    assert test_result_list == ["ScriptTrigger0", "ScriptTrigger1"]
    test_result_list = _converters.convert_repeated_capabilities(
        "ScriptTrigger0,ScriptTrigger1", prefix=r"ScriptTrigger"
    )
    assert test_result_list == ["ScriptTrigger0", "ScriptTrigger1"]
    test_result_list = _converters.convert_repeated_capabilities(
        r"ScriptTrigger0,ScriptTrigger1", prefix="ScriptTrigger"
...
def test_repeated_capabilities_mixed_prefix():
    test_result_list = _converters.convert_repeated_capabilities(
        (slice(0, 1), "2", [4, "5-6"], "7-9", "11:14", "16, 17"), prefix="ScriptTrigger"
    )
    assert test_result_list == [
        "ScriptTrigger0",
        "ScriptTrigger2",
...
def test_repeated_capabilities_slice_prefix():
    test_result_list = _converters.convert_repeated_capabilities(
        slice(0, 1), prefix="ScriptTrigger"
    )
    assert test_result_list == ["ScriptTrigger0"]
    test_result_list = _converters.convert_repeated_capabilities(
        slice(0, 2), prefix="ScriptTrigger"
    )
    assert test_result_list == ["ScriptTrigger0", "ScriptTrigger1"]
    test_result_list = _converters.convert_repeated_capabilities(
        slice(None, 2), prefix="ScriptTrigger"
    )
    assert test_result_list == ["ScriptTrigger0", "ScriptTrigger1"]


def test_repeated_capabilities_without_prefix():
    test_result = _converters.convert_repeated_capabilities_without_prefix(
        (slice(0, 1), "2", [4, "5-6"], "7-9", "11:14", "16, 17")
    )
...
def test_repeated_capabilities_mixed_resource_name():
    test_result_list = _converters.convert_repeated_capabilities_without_prefix(
        ["Dev1", ("Dev2/0", "Dev2/1"), ["Dev3/0:1", "Dev4/1:2"], "Dev5/1"]
    )
...
def test_convert_chained_repeated_capability_to_parts_three_parts():
    chained_rep_cap = (
        "site0/test/PinA,site0/test/PinB,site0/test/PinC,"
...
def test_string_to_list_prefix():
    test_result = _converters._convert_repeated_capabilities(
        ["ScriptTrigger3-ScriptTrigger7"], "ScriptTrigger"
    )
    assert test_result == ["3", "4", "5", "6", "7"]
    test_result = _converters._convert_repeated_capabilities(
        ["ScriptTrigger3:ScriptTrigger7"], "ScriptTrigger"
    ...
    assert test_result == ["3", "4", "5", "6", "7"]
    test_result = _converters._convert_repeated_capabilities(
        ["ScriptTrigger2-ScriptTrigger0"], "ScriptTrigger"
    )
    assert test_result == ["2", "1", "0"]
    test_result = _converters._convert_repeated_capabilities(
        ["ScriptTrigger2:ScriptTrigger0"], "ScriptTrigger"
...
def test_convert_comma_separated_string_to_list():
    out_list = _converters.convert_comma_separated_string_to_list(
        " PinA ,  PinB , PinC  "
    )
