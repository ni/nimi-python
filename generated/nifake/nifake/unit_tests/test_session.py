...
class TestSession(object):
    def setup_method(self, method):
        ...
        self.patched_library_singleton_get = patch(
            "nifake.session._library_singleton.get", return_value=self.patched_library
        )
        ...
        self.tclk_patched_library_singleton_get = patch(
            "nitclk.session._library_singleton.get", return_value=None
        )
        ...
        self.patched_library.niFake_InitWithOptions.side_effect = (
            self.side_effects_helper.niFake_InitWithOptions
        )
        self.disallow_close = self.patched_library.niFake_close.side_effect
        self.patched_library.niFake_close.side_effect = (
            self.side_effects_helper.niFake_close
        )
        ...
        # Mock lock/unlock
        self.LockSession_side_effect_cache = (
            self.patched_library.niFake_LockSession.side_effect
        )
        self.patched_library.niFake_LockSession.side_effect = (
            self.side_effects_helper.niFake_LockSession
        )
        self.side_effects_helper["LockSession"]["callerHasLock"] = True
        self.UnlockSession_side_effect_cache = (
            self.patched_library.niFake_UnlockSession.side_effect
        )
        self.patched_library.niFake_UnlockSession.side_effect = (
            self.side_effects_helper.niFake_UnlockSession
        )
    ...
    def get_ctypes_pointer_for_buffer_side_effect(self, value, library_type=None):
        ret_val = self.get_ctypes_pointer_for_buffer_side_effect_items[
    ...
    def test_init_with_options_and_close(self):
        errors_patcher = patch(
            "nifake.session.errors", spec_set=["handle_error", "_is_error"]
        )
        ...
        self.patched_library.niFake_InitWithOptions.assert_called_once_with(
            _matchers.ViStringMatcher("dev1"),
            _matchers.ViBooleanMatcher(False),
            _matchers.ViBooleanMatcher(False),
            _matchers.ViStringMatcher(""),
            _matchers.ViSessionPointerMatcher(),
        )
        patched_errors.handle_error.assert_called_once_with(
            session,
            self.patched_library.niFake_InitWithOptions.return_value,
            ignore_warnings=False,
            is_error_handling=False,
        )
        session.close()
        self.patched_library.niFake_close.assert_called_once_with(
    ...
    def test_init_with_options_nondefault_and_close(self):
        ...
        self.patched_library.niFake_InitWithOptions.assert_called_once_with(
            _matchers.ViStringMatcher("FakeDevice"),
            _matchers.ViBooleanMatcher(True),
            _matchers.ViBooleanMatcher(True),
            _matchers.ViStringMatcher("Some string"),
            _matchers.ViSessionPointerMatcher(),
        )
        session.close()
        self.patched_library.niFake_close.assert_called_once_with(
    ...
    def test_init_with_error(self):
        ...
        self.side_effects_helper["InitWithOptions"]["vi"] = SESSION_NUM_FOR_TEST
        self.patched_library.niFake_GetError.side_effect = (
            self.side_effects_helper.niFake_GetError
        )
    ...
    def test_lock_session_none(self):
        with nifake.Session("dev1") as session:
            session.lock()
            self.patched_library.niFake_LockSession.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST), None
            )

    def test_unlock_session_none(self):
        with nifake.Session("dev1") as session:
            session.unlock()
            self.patched_library.niFake_UnlockSession.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST), None
            )
    ...
    def test_simple_function(self):
        self.patched_library.niFake_PoorlyNamedSimpleFunction.side_effect = (
            self.side_effects_helper.niFake_PoorlyNamedSimpleFunction
        )
        with nifake.Session("dev1") as session:
            session.simple_function()
            self.patched_library.niFake_PoorlyNamedSimpleFunction.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST)
            )

    def test_self_test(self):
        self.patched_library.niFake_self_test.side_effect = (
            self.side_effects_helper.niFake_self_test
        )
        test_error_code = 0
    ...
    def test_get_a_number(self):
        test_number = 16
        self.patched_library.niFake_GetANumber.side_effect = (
            self.side_effects_helper.niFake_GetANumber
        )
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_GetANumber.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
    ...
    def test_one_input_function(self):
        test_number = 1
        self.patched_library.niFake_OneInputFunction.side_effect = (
            self.side_effects_helper.niFake_OneInputFunction
        )
        with nifake.Session("dev1") as session:
            session.one_input_function(test_number)
            self.patched_library.niFake_OneInputFunction.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViInt32Matcher(test_number),
            )

    def test_vi_int_64_function(self):
        input_value = 1099511627776  # 2^40
        output_value = 2199023255552  # 2^41
        self.patched_library.niFake_Use64BitNumber.side_effect = (
            self.side_effects_helper.niFake_Use64BitNumber
        )
        self.side_effects_helper["Use64BitNumber"]["output"] = output_value
        with nifake.Session("dev1") as session:
            assert session.use64_bit_number(input_value) == output_value
            self.patched_library.niFake_Use64BitNumber.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViInt64Matcher(input_value),
                _matchers.ViInt64PointerMatcher(),
            )

    def test_two_input_function(self):
        test_number = 1.5
        test_string = "test"
        self.patched_library.niFake_TwoInputFunction.side_effect = (
            self.side_effects_helper.niFake_TwoInputFunction
        )
        with nifake.Session("dev1") as session:
            session.two_input_function(test_number, test_string)
            self.patched_library.niFake_TwoInputFunction.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViReal64Matcher(test_number),
                _matchers.ViStringMatcher(test_string),
            )

    def test_get_enum_value(self):
        test_number = 1
        test_turtle = nifake.Turtle.LEONARDO
        self.patched_library.niFake_GetEnumValue.side_effect = (
            self.side_effects_helper.niFake_GetEnumValue
        )
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_GetEnumValue.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
    ...
    def test_get_a_list_enums(self):
        self.patched_library.niFake_EnumArrayOutputFunction.side_effect = (
            self.side_effects_helper.niFake_EnumArrayOutputFunction
        )
        test_list = [1, 1, 0]
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_EnumArrayOutputFunction.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViInt32Matcher(len(test_list)),
                _matchers.ViInt16BufferMatcher(len(test_list)),
            )

    def test_get_a_boolean(self):
        self.patched_library.niFake_GetABoolean.side_effect = (
            self.side_effects_helper.niFake_GetABoolean
        )
        ...
        with nifake.Session("dev1") as session:
            ...
            assert test_result
            self.patched_library.niFake_GetABoolean.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViBooleanPointerMatcher(),
            )

    def test_get_a_list_booleans(self):
        self.patched_library.niFake_BoolArrayOutputFunction.side_effect = (
            self.side_effects_helper.niFake_BoolArrayOutputFunction
        )
        test_list = [1, 1, 0]
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_BoolArrayOutputFunction.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViInt32Matcher(len(test_list)),
                _matchers.ViBooleanBufferMatcher(len(test_list)),
            )

    def test_acquisition_context_manager(self):
        self.patched_library.niFake_Initiate.side_effect = (
            self.side_effects_helper.niFake_Initiate
        )
        self.patched_library.niFake_Abort.side_effect = (
            self.side_effects_helper.niFake_Abort
        )
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_Abort.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST)
            )
        self.patched_library.niFake_close.assert_called_once_with(
            _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST)
    ...
    def test_single_point_read_timedelta(self):
        ...
        test_maximum_time_timedelta = hightime.timedelta(
            nanoseconds=test_maximum_time_ns
        )
        test_reading = 5
        self.patched_library.niFake_Read.side_effect = (
            self.side_effects_helper.niFake_Read
        )
        self.side_effects_helper["Read"]["reading"] = test_reading
        with nifake.Session("dev1") as session:
            assert test_reading == session.read(test_maximum_time_timedelta)
            self.patched_library.niFake_Read.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViReal64Matcher(test_maximum_time_s),
    ...
    def test_enum_input_function_with_defaults(self):
        ...
        self.patched_library.niFake_EnumInputFunctionWithDefaults.side_effect = (
            self.side_effects_helper.niFake_EnumInputFunctionWithDefaults
        )
        with nifake.Session("dev1") as session:
            ...
            calls = [
                call(
                    _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                    _matchers.ViInt16Matcher(0),
                ),
                call(
                    _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                    _matchers.ViInt16Matcher(1),
                ),
            ]  # 0 is the value of the default of nifake.Turtle.LEONARDO, 1 is the value of nifake.Turtle.DONATELLO
            self.patched_library.niFake_EnumInputFunctionWithDefaults.assert_has_calls(
                calls
            )

    def test_string_valued_enum_input_function_with_defaults(self):
        test_mobile_os_name = nifake.MobileOSNames.IOS
        self.patched_library.niFake_StringValuedEnumInputFunctionWithDefaults.side_effect = (
            self.side_effects_helper.niFake_StringValuedEnumInputFunctionWithDefaults
        )
        with nifake.Session("dev1") as session:
            ...
            calls = [
                call(
                    _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                    _matchers.ViStringMatcher("Android"),
                ),
                call(
                    _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                    _matchers.ViStringMatcher("iOS"),
                ),
            ]  # 'ANDROID' is the value of the default of nifake.MobileOSNames.Android, 'iOS' is the value of nifake.MobileOSNames.IOS
            self.patched_library.niFake_StringValuedEnumInputFunctionWithDefaults.assert_has_calls(
                calls
            )

    def test_fetch_waveform(self):
        expected_waveform_list = [1.0, 0.1, 42, 0.42]
        self.patched_library.niFake_FetchWaveform.side_effect = (
            self.side_effects_helper.niFake_FetchWaveform
        )
        self.side_effects_helper["FetchWaveform"][
            "waveformData"
        ] = expected_waveform_list
        self.side_effects_helper["FetchWaveform"]["actualNumberOfSamples"] = len(
            expected_waveform_list
        )
        ...
        expected_waveform_ctypes = ctypes.cast(
            expected_waveform.buffer_info()[0],
            ctypes.POINTER(nifake._visatype.ViReal64 * len(expected_waveform_list)),
        )

        with nifake.Session("dev1") as session:
            self.get_ctypes_pointer_for_buffer_side_effect_items = [
                expected_waveform_ctypes
            ]
            self.get_ctypes_pointer_for_buffer_side_effect_count = 0
            self.patched_library.niFake_WriteWaveform.side_effect = (
                self.side_effects_helper.niFake_WriteWaveform
            )
            with patch(
                "nifake.session.get_ctypes_pointer_for_buffer",
                side_effect=self.get_ctypes_pointer_for_buffer_side_effect,
            ):
        ...
        self.patched_library.niFake_FetchWaveform.assert_called_once_with(
            _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
            _matchers.ViInt32Matcher(len(expected_waveform)),
            _matchers.ViReal64BufferMatcher(expected_waveform),
            _matchers.ViInt32PointerMatcher(),
        )

    def test_fetch_waveform_into(self):
        expected_waveform = [1.0, 0.1, 42, 0.42]
        self.patched_library.niFake_FetchWaveform.side_effect = (
            self.side_effects_helper.niFake_FetchWaveform
        )
    ...
    def test_write_waveform(self):
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_WriteWaveform.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViInt32Matcher(len(expected_waveform)),
                _matchers.ViReal64BufferMatcher(expected_array),
            )
    ...
    def test_return_multiple_types(self):
        self.patched_library.niFake_ReturnMultipleTypes.side_effect = (
            self.side_effects_helper.niFake_ReturnMultipleTypes
        )
        boolean_val = True
        int32_val = 32
        ...
        self.side_effects_helper["ReturnMultipleTypes"][
            "aFloatEnum"
        ] = float_enum_val.value
        ...
        with nifake.Session("dev1") as session:
            (
                ...
                result_array,
                result_string,
            ) = session.return_multiple_types(array_size)
    ...
    def test_multiple_array_types(self):
        self.patched_library.niFake_MultipleArrayTypes.side_effect = (
            self.side_effects_helper.niFake_MultipleArrayTypes
        )
        ...
        self.side_effects_helper["MultipleArrayTypes"][
            "outputArray"
        ] = expected_output_array
        self.side_effects_helper["MultipleArrayTypes"][
            "outputArrayOfFixedLength"
        ] = expected_output_array_of_fixed_length
        with nifake.Session("dev1") as session:
            output_array, output_array_of_fixed_length = session.multiple_array_types(
                output_array_size, input_array_of_floats, input_array_of_integers
            )
            ...
            self.patched_library.niFake_MultipleArrayTypes.assert_called_once_with(
                ...
                _matchers.ViReal64BufferMatcher(
                    len(expected_output_array_of_fixed_length)
                ),
    ...
    def test_multiple_array_types_none_input(self):
        ...
        with nifake.Session("dev1") as session:
            output_array, output_array_of_fixed_length = session.multiple_array_types(
                output_array_size, input_array_of_floats
            )
            assert output_array == output_array
            assert expected_output_array_of_fixed_length == output_array_of_fixed_length
            self.patched_library.niFake_MultipleArrayTypes.assert_called_once_with(
    ...
    def test_multiple_arrays_same_size(self):
        self.patched_library.niFake_MultipleArraysSameSize.side_effect = (
            self.side_effects_helper.niFake_MultipleArraysSameSize
        )
        ...
        with nifake.Session("dev1") as session:
            session.multiple_arrays_same_size(
                input_array_of_floats1,
                input_array_of_floats2,
                input_array_of_floats3,
    ...
    def test_parameters_are_multiple_types(self):
        self.patched_library.niFake_ParametersAreMultipleTypes.side_effect = (
            self.side_effects_helper.niFake_ParametersAreMultipleTypes
        )
        boolean_val = True
        int32_val = 32
        int64_val = 6000000000
        ...
        string_val = "Testing is fun?"
        with nifake.Session("dev1") as session:
            session.parameters_are_multiple_types(
                boolean_val,
                int32_val,
                int64_val,
                enum_val,
            ...
            self.patched_library.niFake_ParametersAreMultipleTypes.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViBooleanMatcher(boolean_val),
                _matchers.ViInt32Matcher(int32_val),
                _matchers.ViInt64Matcher(int64_val),
                _matchers.ViInt16Matcher(enum_val.value),
                _matchers.ViReal64Matcher(float_val),
                _matchers.ViReal64Matcher(float_enum_val.value),
                _matchers.ViInt32Matcher(len(string_val)),
                _matchers.ViStringMatcher(string_val),
            )
    ...
    def test_parameters_are_multiple_types_error(self):
        ...
        self.side_effects_helper["ParametersAreMultipleTypes"][
            "return"
        ] = test_error_code
        self.patched_library.niFake_GetError.side_effect = (
            self.side_effects_helper.niFake_GetError
    ...
    def test_method_with_error(self):
        ...
        self.side_effects_helper["PoorlyNamedSimpleFunction"][
            "return"
        ] = test_error_code
        self.patched_library.niFake_GetError.side_effect = (
            self.side_effects_helper.niFake_GetError
    ...
    def test_error_with_rep_cap(self):
        test_error_code = -42
        test_error_desc = "The answer to the ultimate question"
        self.patched_library.niFake_SetAttributeViReal64.side_effect = (
            self.side_effects_helper.niFake_SetAttributeViReal64
        )
    ...
    def test_get_a_string_of_fixed_maximum_size(self):
        test_string = (
            "A string no larger than the max size of 256 allowed by the function."
        )
        self.patched_library.niFake_GetAStringOfFixedMaximumSize.side_effect = (
            self.side_effects_helper.niFake_GetAStringOfFixedMaximumSize
        )
        self.side_effects_helper["GetAStringOfFixedMaximumSize"][
            "aString"
        ] = test_string
        with nifake.Session("dev1") as session:
            returned_string = session.get_a_string_of_fixed_maximum_size()
            assert returned_string == test_string
            self.patched_library.niFake_GetAStringOfFixedMaximumSize.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViCharBufferMatcher(256),
            )
    ...
    def test_get_a_string_of_size_python_code(self):
        ...
        self.patched_library.niFake_GetAStringUsingPythonCode.side_effect = (
            self.side_effects_helper.niFake_GetAStringUsingPythonCode
        )
        self.side_effects_helper["GetAStringUsingPythonCode"][
            "aString"
        ] = expected_string
        with nifake.Session("dev1") as session:
            returned_string = session.get_a_string_using_python_code(test_size)
            assert returned_string == expected_string
            self.patched_library.niFake_GetAStringUsingPythonCode.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViInt16Matcher(test_size),
                _matchers.ViCharBufferMatcher(test_size),
            )

    def test_return_a_number_and_a_string(self):
        test_string = "this string"
        test_number = 13
        self.patched_library.niFake_ReturnANumberAndAString.side_effect = (
            self.side_effects_helper.niFake_ReturnANumberAndAString
        )
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_ReturnANumberAndAString.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViInt16PointerMatcher(),
                _matchers.ViCharBufferMatcher(256),
            )
    ...
    def test_get_an_ivi_dance_string(self):
        self.patched_library.niFake_GetAnIviDanceString.side_effect = (
            self.side_effects_helper.niFake_GetAnIviDanceString
        )
        string_val = "Testing is fun?"
        ...
        with nifake.Session("dev1") as session:
            ...
            calls = [
                ...
                call(
                    _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                    _matchers.ViInt32Matcher(len(string_val)),
                    _matchers.ViCharBufferMatcher(len(string_val)),
                ),
            ]
    ...
    def test_get_string_ivi_dance_error(self):
        ...
        self.patched_library.niFake_GetAttributeViString.side_effect = (
            self.side_effects_helper.niFake_GetAttributeViString
        )
        self.side_effects_helper["GetAttributeViString"][
            "attributeValue"
        ] = "Testing is fun?"
    ...
    def test_get_an_ivi_dance_with_a_twist_string(self):
        self.patched_library.niFake_GetAnIviDanceWithATwistString.side_effect = (
            self.side_effects_helper.niFake_GetAnIviDanceWithATwistString
        )
        string_val = "Testing is fun?"
        self.side_effects_helper["GetAnIviDanceWithATwistString"][
            "aString"
        ] = string_val
        self.side_effects_helper["GetAnIviDanceWithATwistString"]["actualSize"] = len(
            string_val
        )
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_GetAnIviDanceWithATwistString.assert_has_calls(
                calls
            )
            assert (
                self.patched_library.niFake_GetAnIviDanceWithATwistString.call_count
                == 2
            )

    def test_get_array_using_ivi_dance(self):
        self.patched_library.niFake_GetArrayUsingIviDance.side_effect = (
            self.side_effects_helper.niFake_GetArrayUsingIviDance
        )
    ...
    def test_repeated_capability_method_on_session_timedelta(self):
        ...
        test_maximum_time_timedelta = hightime.timedelta(
            milliseconds=test_maximum_time_ms
        )
        test_reading = 5
        self.patched_library.niFake_ReadFromChannel.side_effect = (
            self.side_effects_helper.niFake_ReadFromChannel
        )
        self.side_effects_helper["ReadFromChannel"]["reading"] = test_reading
        with nifake.Session("dev1") as session:
            value = session.read_from_channel(test_maximum_time_timedelta)
        self.patched_library.niFake_ReadFromChannel.assert_called_once_with(
            _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
            _matchers.ViStringMatcher(""),
            _matchers.ViInt32Matcher(test_maximum_time_ms),
            _matchers.ViReal64PointerMatcher(),
        )
    ...
    def test_device_method_not_exist_on_repeated_capability_error(self):
        with nifake.Session("dev1") as session:
            try:
                ...
                assert (
                    False
                ), "Method has no repeated capability so it shouldn't exist on _RepeatedCapability"
    ...
    def test_chained_repeated_capabilities_list(self):
        with nifake.Session("dev1") as session:
            assert session.sites[0, 1].channels[2, 3]._repeated_capability_list == [
                "site0/2",
                "site0/3",
                "site1/2",
                "site1/3",
    ...
    def test_chained_repeated_capability_method_on_specific_channel(self):
        ...
        with nifake.Session("dev1") as session:
            value = (
                session.sites[0, 1].channels[2, 3].read_from_channel(test_maximum_time)
            )
        self.patched_library.niFake_ReadFromChannel.assert_called_once_with(
            _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
            _matchers.ViStringMatcher("site0/2,site0/3,site1/2,site1/3"),
            _matchers.ViInt32Matcher(test_maximum_time_ms),
            _matchers.ViReal64PointerMatcher(),
        )
        assert value == test_reading
    ...
    def test_get_attribute_int32(self):
        self.patched_library.niFake_GetAttributeViInt32.side_effect = (
            self.side_effects_helper.niFake_GetAttributeViInt32
        )
        test_number = 3
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_GetAttributeViInt32.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(1000004),
                _matchers.ViInt32PointerMatcher(),
            )

    def test_set_attribute_int32(self):
        self.patched_library.niFake_SetAttributeViInt32.side_effect = (
            self.side_effects_helper.niFake_SetAttributeViInt32
        )
    ...
    def test_set_attribute_int32_with_converter(self):
        ...
        with nifake.Session("dev1") as session:
            session.read_write_integer_with_converter = hightime.timedelta(
                milliseconds=test_number_ms
            )
            self.patched_library.niFake_SetAttributeViInt32.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(attribute_id),
                _matchers.ViInt32Matcher(test_number_ms),
            )

    def test_get_attribute_real64(self):
        self.patched_library.niFake_GetAttributeViReal64.side_effect = (
            self.side_effects_helper.niFake_GetAttributeViReal64
        )
        test_number = 1.5
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_GetAttributeViReal64.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(1000001),
                _matchers.ViReal64PointerMatcher(),
            )
    ...
    def test_set_attribute_real64(self):
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_SetAttributeViReal64.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(attribute_id),
                _matchers.ViReal64Matcher(test_number),
        ...
        with nifake.Session("dev1") as session:
            ...
            calls = [
                call(
                    _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                    _matchers.ViStringMatcher(""),
                    _matchers.ViAttrMatcher(1000002),
                    _matchers.ViInt32Matcher(0),
                    None,
                ),
                call(
                    ...
                    _matchers.ViStringMatcher(""),
                    _matchers.ViAttrMatcher(1000002),
                    _matchers.ViInt32Matcher(15),
                    _matchers.ViCharBufferMatcher(len(string)),
                ),
            ]
    ...
    def test_set_attribute_string(self):
        self.patched_library.niFake_SetAttributeViString.side_effect = (
            self.side_effects_helper.niFake_SetAttributeViString
        )
        attribute_id = 1000002
        attrib_string = "This is test string"
        with nifake.Session("dev1") as session:
            session.read_write_string = attrib_string
            self.patched_library.niFake_SetAttributeViString.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(attribute_id),
                _matchers.ViStringMatcher("This is test string"),
            )
        ...
        with nifake.Session("dev1") as session:
            ...
            calls = [
                call(
                    _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                    _matchers.ViStringMatcher(""),
                    _matchers.ViAttrMatcher(1000010),
                    _matchers.ViInt32Matcher(0),
                    None,
                ),
                call(
    ...
    def test_set_attribute_string_with_converter(self):
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_SetAttributeViString.assert_called_once_with(
                ...
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(attribute_id),
                _matchers.ViStringMatcher("42"),
            )

    def test_get_attribute_boolean(self):
        self.patched_library.niFake_GetAttributeViBoolean.side_effect = (
            self.side_effects_helper.niFake_GetAttributeViBoolean
        )
        self.side_effects_helper["GetAttributeViBoolean"]["attributeValue"] = 1
        with nifake.Session("dev1") as session:
            assert session.read_write_bool
            self.patched_library.niFake_GetAttributeViBoolean.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(1000000),
                _matchers.ViBooleanPointerMatcher(),
            )

    def test_set_attribute_boolean(self):
        self.patched_library.niFake_SetAttributeViBoolean.side_effect = (
            self.side_effects_helper.niFake_SetAttributeViBoolean
        )
        attribute_id = 1000000
        attrib_bool = True
        with nifake.Session("dev1") as session:
            session.read_write_bool = attrib_bool
            self.patched_library.niFake_SetAttributeViBoolean.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(attribute_id),
                _matchers.ViBooleanMatcher(True),
    ...
    def test_get_attribute_enum_real64(self):
        ...
        self.side_effects_helper["GetAttributeViReal64"][
            "attributeValue"
        ] = enum_value.value
        with nifake.Session("dev1") as session:
    ...
    def test_set_attribute_enum_real64(self):
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_SetAttributeViReal64.assert_called_once_with(
                ...
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(attribute_id),
                _matchers.ViReal64Matcher(enum_value.value),
            )
    ...
    def test_get_attribute_int64(self):
        self.patched_library.niFake_GetAttributeViInt64.side_effect = (
            self.side_effects_helper.niFake_GetAttributeViInt64
        )
        ...
        with nifake.Session("dev1") as session:
            ...
            assert attr_int == test_number
            self.patched_library.niFake_GetAttributeViInt64.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(attribute_id),
                _matchers.ViInt64PointerMatcher(),
    ...
    def test_set_attribute_int64(self):
        self.patched_library.niFake_SetAttributeViInt64.side_effect = (
            self.side_effects_helper.niFake_SetAttributeViInt64
        )
        attribute_id = 1000006
        test_number = -6000000000
        with nifake.Session("dev1") as session:
            session.read_write_int64 = test_number
            self.patched_library.niFake_SetAttributeViInt64.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(attribute_id),
                _matchers.ViInt64Matcher(test_number),
            )
        ...
        with nifake.Session("dev1") as session:
            try:
                ...
                self.patched_library.niFake_SetAttributeViReal64.assert_called_once_with(
                    ...
                    _matchers.ViStringMatcher(""),
                    _matchers.ViAttrMatcher(1000001),
                    _matchers.ViReal64Matcher(-42.0),
                )
    ...
    def test_add_properties_to_session_error_set(self):
        with nifake.Session("dev1") as session:
            try:
                ...
                assert (
                    str(e)
                    == "'Session' object has no attribute 'non_existent_property'"
                )
    ...
    def test_add_properties_to_repeated_capability_error_set(self):
        with nifake.Session("dev1") as session:
            try:
                ...
                assert (
                    str(e)
                    == "'_SessionBase' object has no attribute 'non_existent_property'"
                )
    ...
    def test_get_error_and_error_message_returns_error(self):
        ...
        self.patched_library.niFake_error_message.side_effect = (
            self.side_effects_helper.niFake_error_message
        )
        self.side_effects_helper["error_message"][
            "errorMessage"
        ] = "Also shouldn't get this"
    ...
    def test_get_error_description_error_message_error(self):
        ...
        with nifake.Session("dev1") as session:
            try:
                ...
                assert e.code == test_error_code
                assert e.description == test_error_desc
        self.patched_library.niFake_error_message.assert_called_once_with(
            _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
            _matchers.ViInt32Matcher(test_error_code),
            _matchers.ViCharBufferMatcher(256),
        )
    ...
    def test_set_custom_type(self):
        self.patched_library.niFake_SetCustomType.side_effect = (
            self.side_effects_helper.niFake_SetCustomType
        )
        cs = nifake.CustomStruct(struct_int=42, struct_double=4.2)
        with nifake.Session("dev1") as session:
            session.set_custom_type(cs)
            self.patched_library.niFake_SetCustomType.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.CustomTypeMatcher(
                    nifake.struct_CustomStruct, nifake.struct_CustomStruct(cs)
                ),
            )

    def test_get_custom_type(self):
        self.patched_library.niFake_GetCustomType.side_effect = (
            self.side_effects_helper.niFake_GetCustomType
        )
    ...
    def test_set_custom_type_array(self):
        self.patched_library.niFake_SetCustomTypeArray.side_effect = (
            self.side_effects_helper.niFake_SetCustomTypeArray
        )
        cs = [
            nifake.CustomStruct(struct_int=42, struct_double=4.2),
            nifake.CustomStruct(struct_int=43, struct_double=4.3),
            nifake.CustomStruct(struct_int=42, struct_double=4.3),
        ]
        cs_ctype = (nifake.struct_CustomStruct * len(cs))(
            *[nifake.struct_CustomStruct(c) for c in cs]
        )
        with nifake.Session("dev1") as session:
            session.set_custom_type_array(cs)
            self.patched_library.niFake_SetCustomTypeArray.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViInt32Matcher(len(cs)),
                _matchers.CustomTypeBufferMatcher(nifake.struct_CustomStruct, cs_ctype),
            )

    def test_get_custom_type_array(self):
        self.patched_library.niFake_GetCustomTypeArray.side_effect = (
            self.side_effects_helper.niFake_GetCustomTypeArray
        )
        cs = [
            nifake.CustomStruct(struct_int=42, struct_double=4.2),
            nifake.CustomStruct(struct_int=43, struct_double=4.3),
    ...
    def test_get_array_using_python_code_double(self):
        ...
        self.patched_library.niFake_GetArraySizeForPythonCode.side_effect = (
            self.side_effects_helper.niFake_GetArraySizeForPythonCode
        )
        self.patched_library.niFake_GetArrayForPythonCodeDouble.side_effect = (
            self.side_effects_helper.niFake_GetArrayForPythonCodeDouble
        )
        array_out = [42.0, 43.0, 44.0]
        array_out_ctype = (nifake._visatype.ViReal64 * len(array_out))(*array_out)
        self.side_effects_helper["GetArraySizeForPythonCode"]["sizeOut"] = len(
            array_out
        )
        self.side_effects_helper["GetArrayForPythonCodeDouble"][
            "arrayOut"
        ] = array_out_ctype
        with nifake.Session("dev1") as session:
    ...
    def test_get_array_using_python_code_custom_type(self):
        ...
        self.patched_library.niFake_GetArrayForPythonCodeCustomType.side_effect = (
            self.side_effects_helper.niFake_GetArrayForPythonCodeCustomType
        )
        cs = [
            nifake.CustomStruct(struct_int=42, struct_double=4.2),
            nifake.CustomStruct(struct_int=43, struct_double=4.3),
        ...
        self.side_effects_helper["GetArrayForPythonCodeCustomType"][
            "arrayOut"
        ] = cs_ctype
        with nifake.Session("dev1") as session:
    ...
    def test_get_cal_date_time(self):
        self.patched_library.niFake_GetCalDateAndTime.side_effect = (
            self.side_effects_helper.niFake_GetCalDateAndTime
        )
        month = 12
        day = 30
        year = 1988
    ...
    def test_get_cal_interval(self):
        self.patched_library.niFake_GetCalInterval = (
            self.side_effects_helper.niFake_GetCalInterval
        )
    ...
    def test_import_attribute_configuration_buffer_list_i8(self):
        self.patched_library.niFake_ImportAttributeConfigurationBuffer.side_effect = (
            self.side_effects_helper.niFake_ImportAttributeConfigurationBuffer
        )
        ...
        with nifake.Session("dev1") as session:
            ...
            self.patched_library.niFake_ImportAttributeConfigurationBuffer.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViInt32Matcher(len(configuration)),
    ...
    def test_export_attribute_configuration_buffer(self):
        self.patched_library.niFake_ExportAttributeConfigurationBuffer.side_effect = (
            self.side_effects_helper.niFake_ExportAttributeConfigurationBuffer
        )
        expected_buffer_list = [
            ord("a"),
            ord("b"),
            ord("c"),
            ord("d"),
        ...
        expected_buffer_ctypes = ctypes.cast(
            expected_buffer.buffer_info()[0],
            ctypes.POINTER(nifake._visatype.ViInt8 * len(expected_buffer_list)),
        )

        self.side_effects_helper["ExportAttributeConfigurationBuffer"][
            "configuration"
        ] = expected_buffer_list
        with nifake.Session("dev1") as session:
            self.get_ctypes_pointer_for_buffer_side_effect_items = [
                expected_buffer_ctypes
            ]
            self.get_ctypes_pointer_for_buffer_side_effect_count = 0
            with patch(
                "nifake.session.get_ctypes_pointer_for_buffer",
    ...
    def test_matcher_prints(self):
        assert (
            _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST).__repr__()
            == "ViSessionMatcher(" + str(nifake._visatype.ViSession) + ", 42)"
        )
        assert (
            _matchers.ViAttrMatcher(SESSION_NUM_FOR_TEST).__repr__()
            == "ViAttrMatcher(" + str(nifake._visatype.ViAttr) + ", 42)"
        )
        assert (
            _matchers.ViInt32Matcher(4).__repr__()
            == "ViInt32Matcher(" + str(nifake._visatype.ViInt32) + ", 4)"
        )
        assert _matchers.ViStringMatcher("0-24").__repr__() == "ViStringMatcher('0-24')"
        assert (
            _matchers.ViReal64Matcher(-42.0).__repr__()
            == "ViReal64Matcher(" + str(nifake._visatype.ViReal64) + ", -42.0)"
        )
        assert (
            _matchers.ViReal64PointerMatcher().__repr__()
            == "ViReal64PointerMatcher(" + str(nifake._visatype.ViReal64) + ")"
        )
        assert (
            _matchers.ViInt32PointerMatcher().__repr__()
            == "ViInt32PointerMatcher(" + str(nifake._visatype.ViInt32) + ")"
        )
        cs = [
            nifake.CustomStruct(struct_int=42, struct_double=4.2),
            nifake.CustomStruct(struct_int=43, struct_double=4.3),
        ...
        assert (
            _matchers.CustomTypeMatcher(
                nifake.struct_CustomStruct, nifake.struct_CustomStruct(cs[0])
            ).__repr__()
            == "CustomTypeMatcher(<class 'nifake.custom_struct.struct_CustomStruct'>, struct_CustomStruct(data=None, struct_int=42, struct_double=4.2))"
        )
        assert (
            _matchers.CustomTypeBufferMatcher(
                nifake.struct_CustomStruct, cs_ctype
            ).__repr__()
            == "CustomTypeBufferMatcher(<class 'nifake.custom_struct.struct_CustomStruct'>, [struct_CustomStruct(data=None, struct_int=42, struct_double=4.2), struct_CustomStruct(data=None, struct_int=43, struct_double=4.3), struct_CustomStruct(data=None, struct_int=42, struct_double=4.3)])"
        )
    ...
    def test_function_name(self):
        with nifake.Session("dev1") as session:
            ...
            assert (
                session.bool_array_output_function.__name__
    ...
    def test_buffer_converter(self):
        self.patched_library.niFake_DoubleAllTheNums.side_effect = (
            self.side_effects_helper.niFake_DoubleAllTheNums
        )
        nums = [1, 2, 3, 4.2]
        nums_x2 = [x * 2 for x in nums]
        with nifake.Session("dev1") as session:
            session.double_all_the_nums(nums)
            self.patched_library.niFake_DoubleAllTheNums.assert_called_once_with(
                _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
                _matchers.ViInt32Matcher(len(nums)),
                _matchers.ViReal64BufferMatcher(nums_x2),
            )

    def test_nitclk_integration(self):
        with nifake.Session("dev1") as session:
            assert (
                str(type(session.tclk)) == "<class 'nitclk.session.SessionReference'>"
            )

    def test_accept_list_of_time_values_as_floats(self):
        self.patched_library.niFake_AcceptListOfDurationsInSeconds.side_effect = (
            self.side_effects_helper.niFake_AcceptListOfDurationsInSeconds
        )
        delays = [-1.5, 2.0]
        with nifake.Session("dev1") as session:
    ...
    def test_return_timedelta(self):
        self.patched_library.niFake_ReturnDurationInSeconds.side_effect = (
            self.side_effects_helper.niFake_ReturnDurationInSeconds
        )
        time_value = -1.5
    ...
    def test_return_timedeltas(self):
        self.patched_library.niFake_ReturnListOfDurationsInSeconds.side_effect = (
            self.side_effects_helper.niFake_ReturnListOfDurationsInSeconds
        )
        time_values = [-1.5, 2.0]
        ...
        self.side_effects_helper["ReturnListOfDurationsInSeconds"][
            "timedeltas"
        ] = time_values_ctype
        with nifake.Session("dev1") as session:
            returned_timedeltas = session.return_list_of_durations_in_seconds(
                len(expected_timedeltas)
            )
...
def test_library_error():
    ...
    try:
        ...
        assert (
            message
            == "A function was not found in the NI-FAKE runtime. Please visit http://www.ni.com/downloads/drivers/ to download a newer version and install it."
        )
