...
class _ScalarMatcher(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, self.expected_type):
            print(
                "{0}: Unexpected type. Expected: {1}. Received: {2}".format(
                    self.__class__.__name__, self.expected_type, type(other)
                )
            )
            return False
        if other.value != self.expected_value:
            print(
                "{0}: Unexpected value. Expected: {1}. Received: {2}".format(
                    self.__class__.__name__, self.expected_value, other.value
                )
            )
            return False
        return True
    ...
    def __repr__(self):
        return "{0}({1}, {2})".format(
            ...
            pp.pformat(self.expected_type),
            pp.pformat(self.expected_value),
        )
...
class _PointerMatcher(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, ctypes.POINTER(self.expected_type)):
            print(
                "Unexpected type. Expected: {0}. Received: {1}".format(
                    ctypes.POINTER(self.expected_type), type(other)
                )
            )
            return False
        return True
    ...
    def __repr__(self):
        return "{0}({1})".format(
            self.__class__.__name__, pp.pformat(self.expected_type)
        )
...
class _BufferMatcher(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, self.expected_type):
            ...
            if not isinstance(other, self.expected_type) and not isinstance(
                other, list
            ):
                print(
                    "Unexpected type. Expected: {0} or {1}. Received: {2}".format(
                        self.expected_type, list, type(other)
                    )
                )
                return False
        if self.expected_size != len(other):
            print(
                "Unexpected length. Expected: {0}. Received: {1}".format(
                    self.expected_size, len(other)
                )
            )
            return False
        if self.expected_value is not None:
            ...
            for i in range(0, len(self.expected_value)):
                if self.expected_value[i] != other[i]:
                    print(
                        "Unexpected value at index {0}. Expected: {1}. Received: {2}".format(
                            i, self.expected_value[i], other[i]
                        )
                    )
                    return False
        return True
    ...
    def __repr__(self):
        return "{0}({1}, {2})".format(
            self.__class__.__name__,
            pp.pformat(self._expected_element_type),
            pp.pformat(self._expected_size_or_value),
        )
...
class ViStringMatcher(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, ctypes.Array):
            ...
            if not isinstance(other, ctypes.Array):
                print(
                    "Unexpected type. Expected: {0}. Received: {1}".format(
                        self.expected_type, type(other)
                    )
                )
                return False
        if (
            len(other) < len(self.expected_string_value) + 1
        ):  # +1 for NULL terminating character
            print(
                "Unexpected length in C string. Expected at least: {0}. Received {1}".format(
                    len(other), len(self.expected_string_value) + 1
                )
            )
            return False
        ...
        if other.value.decode("ascii") != self.expected_string_value:
            print(
                "Unexpected value. Expected {0}. Received: {1}".format(
                    self.expected_string_value, other.value.decode
                )
            )
            return False
        return True
    ...
    def __repr__(self):
        return "{0}({1})".format(
            self.__class__.__name__, pp.pformat(self.expected_string_value)
        )
...
def _compare_ctype_structs(expected, actual):
    ...
    for field in expected._fields_:
        ...
        if expected_val != actual_val:
            print(
                "Unexpected value field {0}. Expected: {1}. Received: {2}".format(
                    field_name, expected_val, actual_val
                )
            )
            return False
    return True
...
class CustomTypeMatcher(object):
    ...
    def __eq__(self, actual):
        if not isinstance(actual, self.expected_type):
            print(
                "Unexpected type. Expected: {0}. Received: {1}".format(
                    self.expected_type, type(actual)
                )
            )
            return False
...
class CustomTypeBufferMatcher(object):
    ...
    def __eq__(self, actual):
        if not isinstance(actual, self.expected_type):
            print(
                "Unexpected array type. Expected: {0}. Received: {1}".format(
                    self.expected_type, type(actual)
                )
            )
            return False
        ...
        if self.expected_value is not None:
            ...
            for a, e in zip(actual, self.expected_value):
                if not isinstance(a, self.expected_element_type):
                    print(
                        "Unexpected type. Expected: {0}. Received: {1}".format(
                            self.expected_element_type, type(a)
                        )
                    )
                    return False
    ...
    def __repr__(self):
        expected_val_repr = (
            "[" + ", ".join([x.__repr__() for x in self.expected_value]) + "]"
        )
        return "{0}({1}, {2})".format(
            self.__class__.__name__,
            pp.pformat(self.expected_element_type),
            expected_val_repr,
        )
...
class ViBooleanMatcher(_ScalarMatcher):
    def __init__(self, expected_value):
        _ScalarMatcher.__init__(
            self, _visatype.ViBoolean, 1 if expected_value is True else 0
        )
