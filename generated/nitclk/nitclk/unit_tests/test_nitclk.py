...
multiple_sessions = [
    ...
    SESSION_NUM_FOR_TEST * 10,
    SESSION_NUM_FOR_TEST * 100,
    SESSION_NUM_FOR_TEST + 1,
]
...
class TestNitclkApi(object):
    def setup_method(self, method):
        ...
        self.patched_library_singleton_get = patch(
            "nitclk.session._library_singleton.get", return_value=self.patched_library
        )
    ...
    def test_initialize_one_session(self):
        self.patched_library.niTClk_Initiate.side_effect = (
            self.side_effects_helper.niTClk_Initiate
        )
    ...
    def test_configure_for_homogeneous_triggers(self):
        self.patched_library.niTClk_ConfigureForHomogeneousTriggers.side_effect = (
            self.side_effects_helper.niTClk_ConfigureForHomogeneousTriggers
        )
    ...
    def test_finish_sync_pulse_sender_synchronize(self):
        min_time = 0.042
        self.patched_library.niTClk_FinishSyncPulseSenderSynchronize.side_effect = (
            self.side_effects_helper.niTClk_FinishSyncPulseSenderSynchronize
        )
        nitclk.finish_sync_pulse_sender_synchronize(
            multiple_session_references, min_time
        )
        self.patched_library.niTClk_FinishSyncPulseSenderSynchronize.assert_called_once_with(
            _matchers.ViUInt32Matcher(len(multiple_sessions)),
            _matchers.ViSessionBufferMatcher(multiple_sessions),
    ...
    def test_is_done(self):
        ...
        self.patched_library.niTClk_IsDone.side_effect = (
            self.side_effects_helper.niTClk_IsDone
        )
    ...
    def test_setup_for_sync_pulse_sender_synchronize(self):
        min_time = 0.042
        self.patched_library.niTClk_SetupForSyncPulseSenderSynchronize.side_effect = (
            self.side_effects_helper.niTClk_SetupForSyncPulseSenderSynchronize
        )
    ...
    def test_synchronize(self):
        min_time = 0.042
        self.patched_library.niTClk_Synchronize.side_effect = (
            self.side_effects_helper.niTClk_Synchronize
        )
    ...
    def test_synchronize_to_sync_pulse_sender(self):
        min_time = 0.042
        self.patched_library.niTClk_SynchronizeToSyncPulseSender.side_effect = (
            self.side_effects_helper.niTClk_SynchronizeToSyncPulseSender
        )
    ...
    def test_wait_until_done(self):
        min_time = 4.2
        self.patched_library.niTClk_WaitUntilDone.side_effect = (
            self.side_effects_helper.niTClk_WaitUntilDone
        )
    ...
    def test_api_error(self):
        ...
        self.patched_library.niTClk_GetExtendedErrorInfo.side_effect = (
            self.side_effects_helper.niTClk_GetExtendedErrorInfo
        )
    ...
    def test_api_get_error_description_fails(self):
        ...
        try:
            ...
            assert (
                e.code == -1
            )  # we want the original error code from getting the attribute.
    ...
    def test_session_reference_error(self):
        ...
        error_string = "Error"
        self.patched_library.niTClk_GetAttributeViReal64.side_effect = (
            self.side_effects_helper.niTClk_GetAttributeViReal64
        )
    ...
    def test_set_vi_real64(self):
        session = nitclk.SessionReference(SESSION_NUM_FOR_TEST)
        self.patched_library.niTClk_SetAttributeViReal64.side_effect = (
            self.side_effects_helper.niTClk_SetAttributeViReal64
        )
        attribute_id = 8
        test_number = 4.2
    ...
    def test_set_vi_string(self):
        session = nitclk.SessionReference(SESSION_NUM_FOR_TEST)
        self.patched_library.niTClk_SetAttributeViString.side_effect = (
            self.side_effects_helper.niTClk_SetAttributeViString
        )
        attribute_id = 13
        test_string = "The answer to the ultimate question is 42"
        session.sync_pulse_sender_sync_pulse_source = test_string
        self.patched_library.niTClk_SetAttributeViString.assert_called_once_with(
            _matchers.ViSessionMatcher(SESSION_NUM_FOR_TEST),
            _matchers.ViStringMatcher(""),
            _matchers.ViAttrMatcher(attribute_id),
    ...
    def test_get_vi_string(self):
        session = nitclk.SessionReference(SESSION_NUM_FOR_TEST)
        self.patched_library.niTClk_GetAttributeViString.side_effect = (
            self.side_effects_helper.niTClk_GetAttributeViString
        )
        attribute_id = 13
        test_string = "The answer to the ultimate question is 42"
        ...
        calls = [
            ...
            call(
                ...
                _matchers.ViStringMatcher(""),
                _matchers.ViAttrMatcher(attribute_id),
                _matchers.ViInt32Matcher(len(test_string)),
                _matchers.ViCharBufferMatcher(len(test_string)),
            ),
        ]
    ...
    def test_set_vi_session_with_int(self):
        session = nitclk.SessionReference(SESSION_NUM_FOR_TEST)
        self.patched_library.niTClk_SetAttributeViSession.side_effect = (
            self.side_effects_helper.niTClk_SetAttributeViSession
        )
    ...
    def test_get_tclk_session_reference(self):
        session = nitclk.SessionReference(SESSION_NUM_FOR_TEST)
        self.patched_library.niTClk_GetAttributeViSession.side_effect = (
            self.side_effects_helper.niTClk_GetAttributeViSession
        )
        attribute_id = 3
        other_session_number = 43
        ...
        assert (
            attr_session_reference._get_tclk_session_reference() == other_session_number
        )
