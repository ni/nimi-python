...
class DriverNotInstalledError(Error):
    ...
    def __init__(self):
        super(DriverNotInstalledError, self).__init__(
            "The NI-Digital Pattern Driver runtime could not be loaded. Make sure it is installed and its bitness matches that of your Python interpreter. Please visit http://www.ni.com/downloads/drivers/ to download and install it."
        )
...
class DriverTooOldError(Error):
    ...
    def __init__(self):
        super(DriverTooOldError, self).__init__(
            "A function was not found in the NI-Digital Pattern Driver runtime. Please visit http://www.ni.com/downloads/drivers/ to download a newer version and install it."
        )
...
class InvalidRepeatedCapabilityError(Error):
    ...
    def __init__(self, invalid_character, invalid_string):
        super(InvalidRepeatedCapabilityError, self).__init__(
            "An invalid character ({0}) was found in repeated capability string ({1})".format(
                invalid_character, invalid_string
            )
        )
...
class SelfTestError(Error):
    ...
    def __init__(self, code, msg):
        self.code = code
        self.message = msg
        super(SelfTestError, self).__init__(
            "Self-test failed with code {0}: {1}".format(code, msg)
        )
