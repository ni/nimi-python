class HistoryRAMCycleInformation(object):
    ...
    def __repr__(self):
        parameter_list = [
            ...
            "expected_pin_states={}".format(
                self._digital_states_representation(self.expected_pin_states)
            ),
            "actual_pin_states={}".format(
                self._digital_states_representation(self.actual_pin_states)
            ),
    ...
    def __str__(self):
        ...
        string_representation += row_format_s.format(
            "Time Set Name", self.time_set_name
        )
        string_representation += row_format_d.format(
            "Vector Number", self.vector_number
        )
        string_representation += row_format_d.format("Cycle Number", self.cycle_number)
        string_representation += row_format_d.format(
            "Scan Cycle Number", self.scan_cycle_number
        )
        string_representation += row_format_s.format(
            "Expected Pin States", self._digital_states_string(self.expected_pin_states)
        )
        string_representation += row_format_s.format(
            "Actual Pin States", self._digital_states_string(self.actual_pin_states)
        )
        string_representation += row_format_s.format(
            "Per Pin Pass Fail", self.per_pin_pass_fail
        )
    ...
    def _digital_states_representation(states):
        states_representation = [
            [
                "{0}.{1}.{2}".format(
                    i.__class__.__module__, i.__class__.__qualname__, i.name
                )
                for i in j
            ]
            for j in states
        ...
        return "[{}]".format(
            ", ".join(["[{}]".format(", ".join(i)) for i in states_representation])
        )
    ...
    def _digital_states_string(states):
        ...
        return "[{}]".format(
            ", ".join(["[{}]".format(", ".join(i)) for i in states_string])
        )
