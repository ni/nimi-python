...
class DriverError(Error):
    '''An error originating from the NI-Sync driver'''
...
class DriverWarning(Warning):
    '''A warning originating from the NI-Sync driver'''
...
class DriverNotInstalledError(Error):
    ...
    def __init__(self):
        super(DriverNotInstalledError, self).__init__('The NI-Sync runtime could not be loaded. Make sure it is installed and its bitness matches that of your Python interpreter. Please visit http://www.ni.com/downloads/drivers/ to download and install it.')
...
def handle_error(session, code, ignore_warnings, is_error_handling):
    ...
    Helper function for handling errors returned by nisync.Library.
