...
def _get_ctype_variable_definition_snippet_for_buffers(parameter, parameters, ivi_dance_step, use_numpy_array, custom_type, module_name):
    '''These are the different cases for initializing the ctype variable for buffers:
        ...
        B510. Input/output numpy array:                                            _converters.convert_buffer_to_ctypes(value=waveform)
        B540. Input buffer (custom type):                                          _converters.convert_buffer_to_ctypes(value=[custom_struct(l) for l in list], library_type=custom_struct)
        B550. Input buffer of simple types:                                        _converters.convert_buffer_to_ctypes(value=array.array('d', list), library_type=visatype.ViReal64)
        B560. Output buffer with mechanism python-code:                            _converters.convert_buffer_to_ctypes(value=array.array('d'), library_type=ViInt32)
        B570. Output buffer with mechanism fixed-size:                             _converters.convert_buffer_to_ctypes(library_type=ViInt32, size=256)
        B580. Output buffer with mechanism ivi-dance, QUERY_SIZE:                  None
        B590. Output buffer with mechanism ivi-dance, GET_DATA:                    _converters.convert_buffer_to_ctypes(value=array.array('d', [0] * buffer_size_ctype.value, library_type=ViInt32)
        B600. Output buffer with mechanism passed-in:                              _converters.convert_buffer_to_ctypes(value-array.array('d', [0] * buffer_size, library_type=ViInt32)
    ...
    if parameter['numpy'] is True and use_numpy_array is True:
        definition = '_converters.convert_buffer_to_ctypes(value={0})  # case B510'.format(parameter['python_name'])
    elif parameter['direction'] == 'in':
        if custom_type is not None:
            definition = '_converters.convert_buffer_to_ctypes([{0}.{1}(c) for c in {2}], library_type={0}.{1})  # case B540'.format(module_name, parameter['ctypes_type'], parameter['python_name'], parameter['python_name'])
        else:
            if parameter['use_array']:
                # If the incoming type is array.array, we can just use that, otherwise we need to create one that is initialized with the iterable
                line1 = 'if {0} is not None:  # case B550'.format(parameter['python_name'])
                line2 = '    if str(type({0})).find("\'array.array\'") != -1:  # case B550'.format(parameter['python_name'])
                line3 = '        {0}_array = {0}  # case B550'.format(parameter['python_name'])
                line4 = '    else:  # case B550'
                line5 = '        {0}_array = array.array("{1}", {0})  # case B550'.format(parameter['python_name'], get_array_type_for_api_type(parameter['ctypes_type']))
                line6 = 'else:  # case B550'
                line7 = '    {0}_array = None  # case B550'.format(parameter['python_name'])
                definitions.append(line1)
                definitions.append(line2)
                definitions.append(line3)
                definitions.append(line4)
                definitions.append(line5)
                definitions.append(line6)
                definitions.append(line7)
                definition = '_converters.convert_buffer_to_ctypes(value={0}_array, library_type={1}.{2})  # case B550'.format(parameter['python_name'], module_name, parameter['ctypes_type'])
            elif parameter['use_list']:
                definition = '_converters.convert_buffer_to_ctypes(value={0}, library_type={1}.{2})  # case B550'.format(parameter['python_name'], module_name, parameter['ctypes_type'])
            else:
        ...
        if parameter['size']['mechanism'] == 'python-code':
            ...
            if parameter['use_array']:
                ...
                definitions.append(line2)
                definition = '_converters.convert_buffer_to_ctypes(value={2}_array, library_type={1}.{3})  # case B560'.format(parameter['ctypes_variable_name'], module_name, parameter['python_name'], parameter['ctypes_type'])
            elif parameter['use_list']:
                definition = '_converters.convert_buffer_to_ctypes(library_type={0}.{1}, size={2}_size)  # case B560'.format(module_name, parameter['ctypes_type'], parameter['python_name'])
            else:
                assert False, "Expected either 'use_array' or 'use_list' to be True. Both False."
        elif parameter['size']['mechanism'] == 'fixed':
            assert parameter['size']['value'] != 1, "Parameter {0} has 'direction':'out' and 'size':{1}... seems wrong. Check your metadata, maybe you forgot to specify?".format(parameter['name'], parameter['size'])
            ...
            if parameter['use_array']:
                ...
                definitions.append(line2)
                definition = '_converters.convert_buffer_to_ctypes(value={2}_array, library_type={1}.{3})  # case B570'.format(parameter['ctypes_variable_name'], module_name, parameter['python_name'], parameter['ctypes_type'])
            elif parameter['use_list']:
                definition = '_converters.convert_buffer_to_ctypes(library_type={0}.{1}, size={2}_size)  # case B570'.format(module_name, parameter['ctypes_type'], parameter['python_name'])
            else:
                assert False, "Expected either 'use_array' or 'use_list' to be True. Both False."
            ...
            if ivi_dance_step == IviDanceStep.QUERY_SIZE:
                ...
                if parameter['use_array']:
                    ...
                    definition = '_converters.convert_buffer_to_ctypes(value={2}_array, library_type={1}.{3})  # case B590'.format(parameter['ctypes_variable_name'], module_name, parameter['python_name'], parameter['ctypes_type'])
                    definitions.append(line2)
                elif parameter['use_list']:
                    definition = '_converters.convert_buffer_to_ctypes(library_type={0}.{1}, size={2}_size)  # case B590'.format(module_name, parameter['ctypes_type'], parameter['python_name'])
                else:
                    assert False, "Expected either 'use_array' or 'use_list' to be True. Both False."
            else:
                assert False, "ivi_dance_step {0} not valid for parameter {1} with ['size']['mechanism'] == 'ivi-dance'".format(ivi_dance_step, parameter['name'])
            ...
            if parameter['use_array']:
                ...
                definition = '_converters.convert_buffer_to_ctypes(value={2}_array, library_type={1}.{3})  # case B600'.format(parameter['ctypes_variable_name'], module_name, parameter['python_name'], parameter['ctypes_type'])
                definitions.append(line2)
            elif parameter['use_list']:
                definition = '_converters.convert_buffer_to_ctypes(library_type={0}.{1}, size={2}_size)  # case B600'.format(module_name, parameter['ctypes_type'], parameter['python_name'])
            else:
                assert False, "Expected either 'use_array' or 'use_list' to be True. Both False."
        else:
...
def test_get_ctype_variable_declaration_snippet_case_b510():
    ...
    assert snippet == ["output_ctype = _converters.convert_buffer_to_ctypes(value=output)  # case B510"]
...
def test_get_ctype_variable_declaration_snippet_case_b540():
    ...
    assert snippet == ["array_in_ctype = _converters.convert_buffer_to_ctypes([custom_struct.custom_struct(c) for c in array_in], library_type=custom_struct.custom_struct)  # case B540"]
...
def test_get_ctype_variable_declaration_snippet_case_b550():
    ...
    expected = [
        ...
        '    if str(type(input_array)).find("\'array.array\'") != -1:  # case B550',
        ...
        'input_array_ctype = _converters.convert_buffer_to_ctypes(value=input_array_array, library_type=visatype.ViReal64)  # case B550',
    ]
...
def test_get_ctype_variable_declaration_snippet_case_b560():
    ...
    expected = [
        ...
        'array_out_ctype = _converters.convert_buffer_to_ctypes(library_type=custom_struct.custom_struct, size=array_out_size)  # case B560',
    ]
    assert len(actual) == len(expected)
    for i in range(max(len(actual), len(expected))):
        assert actual[i] == expected[i]
...
def test_get_ctype_variable_declaration_snippet_case_b570():
    ...
    expected = [
        ...
        'an_int_ctype = _converters.convert_buffer_to_ctypes(value=an_int_array, library_type=visatype.ViInt16)  # case B570',
    ]
    assert len(actual) == len(expected)
    for i in range(max(len(actual), len(expected))):
        assert actual[i] == expected[i]
...
def test_get_ctype_variable_declaration_snippet_case_b600():
    ...
    expected = [
        ...
        'output_ctype = _converters.convert_buffer_to_ctypes(value=output_array, library_type=visatype.ViInt64)  # case B600',
    ]
    assert len(actual) == len(expected)
    for i in range(max(len(actual), len(expected))):
        assert actual[i] == expected[i]
