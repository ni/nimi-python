...
% endfor
    ...
    def __init__(self, resourceName, idQuery = 0, reset = False, optionString = "", mocking = False):
        ...
        new_session = ctypes.c_ulong(self.vi)
        error_code = self.library.${c_function_prefix}InitWithOptions(resourceName.encode('ascii'), idQuery, reset, optionString.encode('ascii'), ctypes.pointer(new_session))
        self.vi = new_session.value
