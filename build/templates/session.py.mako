...
% endif
    def __init__(self, repeated_capability_list, all_channels_in_session, ${config['session_handle_parameter_name']}, library_interpreter, encoding, freeze_it=False):
...
class Session(_SessionBase):
    ...
    def __init__(${init_method_params}):
        ...
        super(Session, self).__init__(
            ...
            ${config['session_handle_parameter_name']}=0,
...
% endfor
        ...
        # Note that we set _${config['session_handle_parameter_name']} to 0 in SessionBase, so that if
        # ${init_function['python_name']} fails, the error handler can reference it.
        # And then once ${init_function['python_name']} succeeds, we can update _${config['session_handle_parameter_name']}
