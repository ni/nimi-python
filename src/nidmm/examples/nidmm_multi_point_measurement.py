...
parser = argparse.ArgumentParser(description='Performs a multi point measurement using the NI-DMM API.', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
try:
    with nidmm.Session(args.name) as session:
        ...
        session.configure_multi_point(args.trigger_count, args.samples, nidmm.SampleTrigger.IMMEDIATE, args.timeout)
        measurements, numberOfMeasurements = session.read_multi_point(args.timeout, args.samples)
