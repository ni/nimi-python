...
<%
    ...
    if f['session_name'] == 'fetch_multiple':
    ...
    elif f['session_name'] == 'measure_multiple':
        param_list = ''
        ...
        raise ValueError('Only fetch_multiple and measure_multiple are supported. Got {0}'.format(f['session_name']))
%>\
    def ${f['session_name']}${suffix}(${helper.get_params_snippet(f, helper.ParameterUsageOptions.SESSION_METHOD_DECLARATION)}):
        ...
        voltage_measurements, current_measurements${in_compliances_return} = self._${f['session_name']}(${param_list})
