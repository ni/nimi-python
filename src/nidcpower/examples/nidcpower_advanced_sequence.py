...
def example(argsv):
    ...
    parser.add_argument('-v0', '--voltage_start', default=1.0, type=float, help='Voltage level at which sweep starts (V)')
    parser.add_argument('-vf', '--voltage_final', default=3.0, type=float, help='Voltage level at which sweep ends (V)')
    parser.add_argument('-c0', '--current_start', default=100e-6, type=float, help='Current level at which sweep starts (A)')
    parser.add_argument('-cf', '--current_final', default=300e-6, type=float, help='Current level at which sweep ends (A)')
    ...
    with nidcpower.Session(resource_name=args.resource_name, channels=args.channels, option_string=args.option_string) as session:
        ...
        with session.initiate():
            ...
            voltage_measurements, current_measurements, in_compliance = session.fetch_multiple(count=args.steps * 2)
